...
//        } catch (IOException e) {
//        	Log.error("[人员导入]异常：{}",e.getMessage());
...
//        }
  ...
  public List<T> importExcel2007(String sheetName, InputStream input) throws Exception {
      int maxCol = 0;
      List<T> list = new ArrayList<T>();
//        try {  SXSSFWorkbook
...
//                        	c = String.valueOf(cell.getNumericCellValue());
                          ...
                          if(HSSFDateUtil.isCellDateFormatted(cell)){// 判断单元格是否属于日期格式
                              c = DateUtil.dateToDateString(cell.getDateCellValue());//java.util.Date类型
                          }else{
                              // 整数后面加.0
                              long longVal = Math.round(cell.getNumericCellValue());
                              if(Double.parseDouble(longVal + ".0") == cell.getNumericCellValue()) {
                                  c = String.valueOf(longVal);
                              }else{
                                  c = String.valueOf(cell.getNumericCellValue());
                              }
                          }
...
//        }
  ...
  public boolean exportExcel2007(List<T> lists[], String sheetNames[],
          OutputStream output) {
      if (lists.length != sheetNames.length) {
          System.out.println("数组长度不一致");
          return false;
...
/*                // 如果设置了提示信息则鼠标放上去提示.
  ...
  public boolean exportExcel2007(List<T> list, String sheetName,
          OutputStream output) {
      List<T>[] lists = new ArrayList[1];
      lists[0] = list;
      ...
      return exportExcel2007(lists, sheetNames, output);
  }
